...
export class CongressAPI {
  ...
  async searchBills(query: string, congress?: number, limit: number = 20): Promise<CongressBill[]> {
    try {
  ...
  async getBillDetails(congress: number, billType: string, billNumber: number): Promise<CongressBill | null> {
    try {
      const params = new URLSearchParams({
        format: 'json',
        ...(this.apiKey && { api_key: this.apiKey })
  ...
  async getRecentBills(congress?: number, limit: number = 20): Promise<CongressBill[]> {
    try {
      const params = new URLSearchParams({
        limit: limit.toString(),
        format: 'json',
...
export class FederalRegisterAPI {
  async searchDocuments(query: string, limit: number = 20): Promise<FederalRegisterDocument[]> {
    try {
      const params = new URLSearchParams({
        q: query,
  ...
  async getRecentDocuments(limit: number = 20): Promise<FederalRegisterDocument[]> {
    try {
      const params = new URLSearchParams({
        per_page: limit.toString(),
        order: 'newest'
  ...
  async getDocumentDetails(documentNumber: string): Promise<FederalRegisterDocument | null> {
    try {
...
export class USCodeAPI {
  async searchCode(query: string, title?: number, limit: number = 20): Promise<USCodeSection[]> {
    try {
      const params = new URLSearchParams({
        q: query,
        limit: limit.toString(),
      ...
      return response.data.results?.map((section: any) => ({
        title: section.title,
  ...
  async getSection(title: number, section: string): Promise<USCodeSection | null> {
    try {
...
export class RegulationsGovAPI {
  ...
  async searchComments(query: string, limit: number = 20): Promise<RegulationComment[]> {
    try {
      const params = new URLSearchParams({
        q: query,
